                ...
                <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 min-w-[400px] min-h-max bg-white p-5 flex flex-col gap-4 justify-between">
                    ...
                    <form className='w-full flex flex-col gap-4' encType='multipart/form-data'>
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                type="email"
                                className={`border-2 w-full outline-none px-4 py-2 ${errors.user_email && "border-red-400"}`}
                                {...register("user_email", {
                                    required: true
                                })}
                            />
                            {errors.user_email && <span className='text-red-400'>Email is required</span>}
                        </div>
                        <div className="flex flex-col gap-2">
                            <label htmlFor="password" className='font-medium'>Password</label>
                            <input
                                type="password"
                                className={`border-2 w-full outline-none px-4 py-2 ${errors.user_password && "border-red-400"}`}
                                {...register("user_password", {
                                    required: true
                                })}
                            />
                            {errors.user_password && <span className='text-red-400'>Password is required</span>}
                        </div>
                        <Button
                            type="main"
                            size="l"
                    ...
                    <div className="flex justify-between">
                        ...
                        <Link to={"/register"}><h3 className='text-base'>Register</h3></Link>
                    </div>
                </div>
            </div>
        </Wrapper>
