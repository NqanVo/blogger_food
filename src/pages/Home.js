...
const Home = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axios.get(`${process.env.REACT_APP_URL_API}posts?category=${choseCat}&pages=${viewPageCat}&limit=6`)
            ...
            <section className="w-full mx-auto px-3 md:px-5 lg:px-32 xl:px-40 grid grid-cols-4 xl:gap-4 gap-3 xl:my-4 my-3 min-h-screen">
                <div className="col-span-4 lg:col-span-3 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-4">
                        <h1 className='pb-2 border-b-4 border-red-400'>Most Popular Dish</h1>
                        <div className="w-full grid grid-cols-5 gap-2">
                            {dataMostPopularDish ? dataMostPopularDish.map((post, index) => {
                                return (
                                    <div
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex justify-end items-center gap-2 pb-2 border-b-4 border-red-400 font-medium">
                            ...
                            <Button
                                size="s"
                                name="Viet"
                                type={choseCat === "viet nam" ? "main" : "primary"}
                                handleOnClick={() => setChoseCat("viet nam")}
                            ></Button>
                            <Button
                                size="s"
                                name="China"
                            ...
                            {dataCategory.map((item, index) => {
                                return (
                                    <div key={index} className={`relative w-full h-40 overflow-hidden`}>
                                        <img src={process.env.REACT_APP_URL_API_IMAGE + item.post_thumb} alt="" className='absolute inset-0 w-full h-full object-cover' />
                                        <div className="absolute inset-0 w-full h-full m-auto bg-black/40 blur-[2px]" />
                                    </div>
                                )
                            })}
                ...
                <div className="col-span-4 lg:col-span-1 flex flex-col gap-4">
                    ...
                    <div className="hidden lg:flex flex-col gap-4 text-center">
                        ...
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Recusandae quo consectetur voluptates laboriosam ea deserunt obcaecati ad illum.</p>
                    </div>
                    {/* news */}
                    <News
                        data={categorise}
