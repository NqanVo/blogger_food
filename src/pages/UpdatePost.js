...
const UpdatePost = memo(() => {
    ...
    useEffect(() => {
        ...
        const getPost = async () => {
            const res = await axios.get(process.env.REACT_APP_URL_API + `posts/${post_id}`)
    ...
    const handleSubmitPost = (values) => {
        ...
        UpdatePostAPI(user_id, post_id, formData, navigate, dispath)
    ...
    return (
        <Wrapper>
            <Header />
            {
                !user_id ? <Button name={"Login"} handleOnClick={() => navigate("/login")} size="l" type="main" />
                    :
                            ...
                            (<FormC handleSubmit={handleSubmit} onSubmit={handleSubmitPost}>
                                <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 min-h-screen">
                                    <div className="col-span-1 lg:col-span-3 flex flex-col gap-2">
                                        <img src={cruAvatar || process.env.REACT_APP_URL_API_IMAGE + dataPost.post_thumb} alt="" className='w-full h-96 object-cover' />
                                        <InputC
                                            type={"text"}
                                            label={"Title"}
                                            name={"post_title"}
