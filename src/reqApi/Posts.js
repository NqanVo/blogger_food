...
export const UpdatePostAPI = async (user_id, post_id, formData, navigate, dispath) => {
    const res = await axios.put(process.env.REACT_APP_URL_API + `posts/update-post/${user_id}/${post_id}`, formData)
    if (!checkTokenExpired(res, dispath)) {
        Notify(res.data.status, res.data.message)
        // navigate("/information")
    }
