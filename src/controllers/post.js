...
const removeThumbOld = async (post_id) => {
    const post = await db.tbl_post.findByPk(post_id, { attributes: ['post_thumb'] })
...
const getPosts = async (req, res) => {
    try {
        ...
        const Au = author ? await db.tbl_user.findAll({
        ...
        const Cat = category ? await db.tbl_category.findAll({
        ...
        let page = (pages == 0 || pages == 1 || !pages) ? 0 : parseInt(pages * limit) - limit
        let orderBy = ""
        switch (order) {
            case "DESC":
                orderBy = "DESC"
...
const createPost = async (req, res) => {
    try {
        ...
        const rename_thumb = await post_thumb.name.split(".")[0] + "_" + Date.now() + "." + post_thumb.mimetype.split("/")[1]
        await db.tbl_post.create({
            post_title: post_title,
            post_desc: post_desc,
            post_thumb: rename_thumb,
            category_id: category_id,
            user_id: user_id
        })
        post_thumb.mv('./src/uploads/' + rename_thumb)
